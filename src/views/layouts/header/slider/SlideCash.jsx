...
const SlideCash = iniClasses => {
	...
	let steleLoad = BaseInformation.SlideInfo.map((info, index) => {
		switch (index) {
			case 0:
				...
				break
			case 1:
				styleList.push({ id: info.id, condition: 'wait_slide_right' })
				break
			default:
				...
				} else styleList.push({ id: info.id, condition: 'wait_slide_left' })
				break
		}
	...
	return (
		...
		<div id="slider_body">
			<button className={'111'} onClick={rerenderSt}>
				клик
			</button>
			<div id="slide_prevew">
				<SlidePrevew />
			...
			<div id="slide_discription">
				<div className="first_line">
					{BaseInformation.SlideInfo.map((info, index) => (
						<span
							data={info.id}
							className="first_line active_slide"
							key={index.toString()}
						>
							{info.first_line}
						</span>
