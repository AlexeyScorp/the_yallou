...
const Slide = (iniClasses) => {
    ...
    const discriptionLoadFirstLine =
        BaseInformation.SlideInfo.map((info, index) =>
            <span data={info.id} className="" key={index.toString()}>
                {info.first_line}
            </span>);

    const discriptionLoadSecondLine =
        BaseInformation.SlideInfo.map((info, index) =>
            <span data={info.id} className="" key={index.toString()}>
                {info.second_line}
            </span>);
    ...
    const SlidePrevew = () => {
        return (
            <div id="slide_prevew" className="slider">
                <div className="slide_image wait_slide_left"
                    key={prevImgIndex}>
                    {imgDiv[prevImgIndex]}
                </div>
                ...
                <div className="slide_image wait_slide_right"
                    key={nextImgIndex}>
                    {imgDiv[nextImgIndex]}
                </div>
            </div >
        ...
        return (
            <div id="slide_discription">
                <div className="first_line">
                    {imgDiscriptionFirst[activeIndex]}
                </div>
                <div className="second_line">
                    {imgDiscriptionSecond[activeIndex]}
                </div>
            </div>
        )
    }
